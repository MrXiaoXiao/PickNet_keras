...
# generator class using DiTing, INSTANCE, and STEAD datasets
class SimpleTrainGenerator_DIS(tf.keras.utils.Sequence):
    """
    ...
    def __init__(self, 
        ...
        """
        # load DiTing Params
        self.DiTing_hdf5_path = init_dict['DiTing_hdf5_path']
        self.DiTing_csv_path = init_dict['DiTing_csv_path']
        self.DiTing_csv = pd.read_csv(self.DiTing_csv_path, dtype = {'key': str})
        self.DiTing_indexes = np.arange(len(self.DiTing_csv),dtype=np.int)
        ...
        self.STEAD_ev_indexes = np.arange(len(self.STEAD_ev_csv),dtype=np.int)
        
        self.STEAD_noise_csv_path = init_dict['STEAD_noise_csv_path']
        self.STEAD_noise_csv = pd.read_csv(self.STEAD_noise_csv_path)
        self.STEAD_no_indexes = np.arange(len(self.STEAD_noise_csv),dtype=np.int)
        ...
        self.INSTANCE_ev_indexes = np.arange(len(self.INSTANCE_ev_csv),dtype=np.int)

        # batch Params
        self.DiTing_batch_size = init_dict['DiTing_batch_size']
    ...
    def __getitem__(self, index):
        ...
        # get from DiTing dataset
        total_choice_ids = self.DiTing_indexes[index*self.DiTing_batch_size:(index+1)*self.DiTing_batch_size]
        for batch_dx in range(start_dx, start_dx + self.DiTing_batch_size):
            choice_id = total_choice_ids[int(batch_dx-start_dx)]
            choice_line = self.DiTing_csv.iloc[choice_id]
            ...
            temp_data_X, temp_data_Y = get_instance_for_training(dataset='DiTing',
                                                                dataset_path=self.DiTing_hdf5_path,
        ...
        start_dx += self.DiTing_batch_size
        
        # get from INSTANCE dataset
        total_choice_ids = self.INSTANCE_ev_indexes[index*self.INSTANCE_batch_size:(index+1)*self.INSTANCE_batch_size]
        ...
        total_choice_ids = self.STEAD_ev_indexes[index*self.STEAD_batch_size:(index+1)*self.STEAD_batch_size]
        for batch_dx in range(start_dx, start_dx + self.STEAD_batch_size):
            choice_id = total_choice_ids[int(batch_dx-start_dx)]
            choice_line = self.STEAD_ev_csv.iloc[choice_id]
            key = choice_line['trace_name']
        ...
        total_choice_ids = self.STEAD_no_indexes[index*self.Noise_batch_size:(index+1)*self.Noise_batch_size]
        for batch_dx in range(start_dx, start_dx + self.Noise_batch_size):
            choice_id = total_choice_ids[int(batch_dx - start_dx)]
            choice_line = self.STEAD_noise_csv.iloc[choice_id]
            key = choice_line['trace_name']
            data, _ , _ = get_from_STEAD(key)
    ...
    def on_epoch_end(self):
        ...
        epoch end
        """
        shuffle(self.DiTing_indexes)
        shuffle(self.INSTANCE_ev_indexes)
        shuffle(self.STEAD_ev_indexes)
        shuffle(self.STEAD_no_indexes)
        return

# generator class using DiTing, INSTANCE, and STEAD datasets
class SimpleTrainGenerator_DIS_Parallel(tf.keras.utils.Sequence):
    """
    generator data for PickNet Keras Traning
    """
    def __init__(self, 
